        ...
        <div className="mb-6">
          <h3 className="text-xl font-semibold text-white mb-2">
            ...
            <span className="text-amber-400">
              {productInfo?.brandId?.brand}
            </span>
          </h3>
          <h5 className="text-lg text-gray-300">
            Sub-Category:{" "}
            <span className="text-amber-400">
              {productInfo?.subCategoryId?.subCategory}
            </span>
          </h5>
        </div>
      )}
        ...
        <div className="space-y-4">
          {variantInfo.map((variant, index) => (
            <div key={index} className="bg-gray-700 rounded-md p-4 ">
              <h2>
                {productInfo.activeOffer !== 0 && (
                  <>
                    <span>{productInfo.activeOffer}</span>
                    {productInfo.activeOfferType === "percentage" && (
                      <span>%</span>
                    )}
                    {productInfo.activeOfferType === "flat" && (
                      <span>Flat</span>
                    )}
                  </>
                )}
              ...
              <h5 className="text-md text-gray-300 mb-4">
                Quantity:{" "}
                <span className="text-amber-400">{variant.quantity}</span>
              </h5>
            </div>
          ))}
        </div>
